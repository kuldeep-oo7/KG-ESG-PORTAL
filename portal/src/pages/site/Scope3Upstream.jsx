...
export default function Scope3Upstream() {
  ...
  // tonne.km basis multiplies weight × distance; per-vehicle (km/miles) uses distance only.
  const qty = /tonne/i.test(u) ? (parseFloat(tonnes) || 0) * (parseFloat(distance) || 0) : (parseFloat(distance) || 0)
  const tco2ePreview = +(qty * ef / 1000).toFixed(6)
  const preview = vehicleType && distance ? { tco2e: tco2ePreview } : null
