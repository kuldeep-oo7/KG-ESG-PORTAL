...
export default function Scope1Mobile() {
  ...
  const preview = type && unit && consumption ? calcMobile(type, unit, parseFloat(consumption) || 0) : null

  function buildEntry() {
    if (!type || !unit || !consumption) return null
    const { ef, tco2e } = calcMobile(type, unit, parseFloat(consumption))
    const e = { date: new Date().toISOString().slice(0, 10), Type: type, Unit: unit, Consumption: consumption, Source: 'Defra v1.0', 'Emission Factor': ef, remarks, ef, tco2e }
      ...
      fields={({ onSubmit }) => (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <Select label="Type" value={type} onChange={setType} options={MOBILE_TYPES} required />
            <Select label="Unit of Measurement" value={unit} onChange={setUnit} options={MOBILE_UNITS} required />
